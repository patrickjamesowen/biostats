...
import scipy as sp
k = 22 #homozygous dominant 'AA'
m = 17 #hetrozygous 
n = 17 #homozygous recessive 'aa'

total_pop = 4*sp.misc.comb(k+m+n, 2)


#mpunnett squares - should do this in a far more matrixy way

def numberofdoms(parent1, parent2):
    numb = 0
...
total_doms = (numberofdoms(dom, dom) * sp.misc.comb(k, 2)) + (numberofdoms(dom, het) * k * m) + (numberofdoms(dom, rec) * k * n) + (numberofdoms(het, het) * sp.misc.comb(m, 2)) + (numberofdoms(het, rec) * m * n)

prob_doms = total_doms / total_pop
